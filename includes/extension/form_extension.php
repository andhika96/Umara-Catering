<?php
	 ...
	 *	Information file: Form Extension
	 */
	...
	if ( ! function_exists('form_open'))
	{
		...
		{
			$config =& load_class('Config', 'system');
			$uri =& load_class('URI', 'system');
			$security =& load_class('Security', 'system');

			// If no action is provided then set to the current url
			if ( ! $action)
			{
				$action = $config->site_url($uri->uri_string());
			}
			...
			if ($config->item('csrf_protection') === TRUE && strpos($action, $config->base_url()) !== FALSE && ! stripos($form, 'method="get"'))
			{
				// Prepend/append random-length "white noise" around the CSRF
				// token input, as a form of protection against BREACH attacks
				if (FALSE !== ($noise = $security->get_random_bytes(1)))
				{
