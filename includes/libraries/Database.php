...
class ARUNA_Database {
	...
	function __construct($active_group = '') 
	{
		...
		$this->conn = $this->sql_connect($db[$active_group]['host'], $db[$active_group]['user'], $db[$active_group]['password'], $db[$active_group]['dbname']);
		
		return $this->conn;
	}
	
	public function sql_connect($host, $user, $password, $db) 
	{
		switch ($this->dbtype) 
		{
			case "PDO_MySQL":
				try 
				{
					$dbi = new PDO("mysql:host=$host;dbname=$db", $user, $password);
			...
			case "PDO_PostgreSQL":
				try 
				{
					$dbi = new PDO("pgsql:host=$host;dbname=$db", $user, $password);
					$dbi->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
					$dbi->setAttribute(PDO::ATTR_EMULATE_PREPARES, FALSE);
				}
				catch (PDOExecption $e) 
	...
	public function sql_prepare($query, $type_query = NULL) 
	{
		switch ($this->dbtype) 
		{
			case 'PDO_MySQL':
	...
	public function sql_bindParam($att, $stmt) 
	{
		switch ($this->dbtype) 
		{
			case "PDO_MySQL":
	...
	public function sql_fetch_array(&$res, $nr = 0) 
	{
		switch ($this->dbtype) 
		{
			case "PDO_MySQL":
	...
	public function num_rows($table = '', $alias = '', $params = '')
	{
		if (empty($table))
		{
			show_error('Can not get table name database');
		}
	...
	public function sql_insert($att, $table_name) 
	{
		switch ($this->dbtype) 
		{
			case "PDO_MySQL":
	...
	public function sql_delete($table_name, $arg = 0) 
	{
		switch ($this->dbtype) 
		{
			case 'PDO_MySQL':
